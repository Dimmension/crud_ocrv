...
def create_plans(user_id: str, place: str, time: str):
    ...
    if user_id is None or place is None or time is None:
        ...
        with conn:
            cursor: sqlite3.Cursor = conne.cursor()

            # Запрос к таблице "plans" базы данных (вставка новой записи).
            sql_query: str = """INSERT INTO plans (user_id, place, time)
            VALUES (?, ?, ?)"""
...
def read_plans(user_time: str) -> list:
    ...
    with conn:
        ...
        sql_query: str = """SELECT * FROM plans WHERE time LIKE (?)"""
...
def update_plans(place: str, old_time: str, new_time: str) -> bool:
    ...
    with conn:
        ...
        # базы данных.
        sql_query: str = """SELECT * FROM plans WHERE place = (?) AND
        time = (?)"""
        ...
        if result:
            sql_query: str = """UPDATE plans SET time = (?) WHERE time = (?)
            AND place = ?"""
...
def delete_plans(place: str, time: str) -> bool:
    ...
    with conn:
        ...
        if place is None or time is None:
            ...
            if result:
                cursor: sqlite3.Cursor = conn.cursor()
                sql_query: str = """DELETE from plans WHERE place = ? AND
                time = ?"""
                arguments: tuple = (place, time)
                cursor.execute(sql_query, arguments)
                return True
